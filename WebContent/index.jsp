...
机型:
<select name="mairnoid">
	<option value="0">请选择</option>
	<c:forEach items="${showMairno}" var="mairno">
		<option value="${mairno.id }">${mairno.airtype }</option>
	</c:forEach>
</select>
机长:
<select name="leaderid">
	<option value="0">请选择</option>
	<c:forEach items="${showLeaderName}" var="leadername">
		<option value="${leadername.id }">${leadername.leaderName }</option>
	</c:forEach>
</select>
